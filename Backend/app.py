...
def live_search():
  data = request.get_json()
  if data is None:
    print('\nERROR: LiveSearch: No data provided')
    return jsonify({'success': False, 'message': 'No data provided'}), 400
  # user_id = get_user_id()
  # if not user_id:
  #   print('\nERROR: LiveSearch: User ID is not in session')
  ...
  try:
    ...
    return jsonify({
      'success': True, 
      'message': 'Infringement analysis completed', 
      'infringement_analysis': infringement_analysis_results,
      'execution_time': f"{time_in_hours}h {time_in_minutes}m {time_in_seconds}s"
      }), 200
  except Exception as e:
    ...
    return jsonify({
      'success': False, 
      'message': f'Error performing infringement analysis: {str(e)}',
      'search_results': searchResults,
      'execution_time': f"{time_in_hours}h {time_in_minutes}m {time_in_seconds}s"
      }), 500
...
def search_product_sources():
  ...
  serialized = []
  for item in tqdm(product_details_list, desc="Serializing Product Details"):
    if hasattr(item, 'model_dump'):
